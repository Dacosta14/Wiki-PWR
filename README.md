#  MineResumo

MineResumo é uma wiki temática sobre o jogo **Minecraft**, desenvolvida em **HTML** e **CSS**, com o objetivo de reunir informações importantes de forma organizada, visual e acessível.

O projeto foi criado como trabalho acadêmico e busca auxiliar jogadores iniciantes e fãs do jogo com conteúdos explicativos sobre mecânicas, criaturas e história do universo Minecraft.

---

## Páginas do Projeto

###  Home

Página inicial com apresentação da wiki e navegação principal.

### 📖 História

Conta a trajetória do Minecraft desde sua criação até se tornar um dos jogos mais populares do mundo.

###  Nether

Explica a dimensão Nether, como acessá-la, criaturas presentes, recursos raros e curiosidades.


---

##  Tecnologias Utilizadas

* HTML5
* CSS3

---

##  Objetivo do Projeto

* Praticar desenvolvimento web front-end
* Estruturar múltiplas páginas interligadas
* Trabalhar estilização visual com CSS
* Criar uma wiki temática organizada
* Melhorar experiência do usuário através de design intuitivo

---

##  Recursos Visuais

* Menu fixo de navegação
* Fundos personalizados por página
* Layout responsivo
* Cards informativos
* Efeitos hover no menu
* Estilo inspirado em games/wiki

---

##  Estrutura de Pastas

```bash
MineResumo/
│── paginas/
│   ├── index.html
│   ├── historia.html
│   ├── nether.html
│
│── css/
│   ├── home.css
│   ├── historia.css
│   ├── nether.css
│
│── imagens/
```

---

## Como Executar

1. Baixe ou clone este repositório
2. Abra a pasta do projeto
3. Execute o arquivo `index.html` no navegador

---

##  Melhorias Futuras

* Adicionar JavaScript interativo
* Mais páginas de mobs e biomas
---
